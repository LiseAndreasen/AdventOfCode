...
function gradeHands() {
	...
	foreach($hands as $key => $hand) {
		...
		// if there are jokers, add them to the other card value with the highest count
		if(isset($uniq["0"])) {
			$handSize = sizeof($uniq);
			$jokerSize = $uniq["0"];
			unset($uniq["0"]);
			sort($uniq);
			$uniq[$handSize - 2] += $jokerSize;
		} else {
			sort($uniq);
		}
		// 5 of a kind
...
foreach(preg_split("/((\r?\n)|(\r\n?))/", $input) as $line) {
	if(strlen($line)>2) {
		$tmp1 = explode(" ", $line);
		// exchange AKQTJ with EDCB0
		$tmp2 = $tmp1[$theHand];
		$tmp2 = str_replace("A", "E", $tmp2);
		$tmp2 = str_replace("K", "D", $tmp2);
		$tmp2 = str_replace("Q", "C", $tmp2);
		$tmp2 = str_replace("T", "B", $tmp2);
		$tmp2 = str_replace("J", "0", $tmp2);
		$tmp1[$theHand] = $tmp2;
		$hands[] = $tmp1;
	}
}
